...
save "E:\PLFS\HH FV &RV.dta", replace


*if commands on 11/11/2019
gen HHT_recoded= 11 if HH_type==1
drop HHT_recoded
gen HHT_recoded= 11 if HH_type==1 and Sector==1
gen HHT_recoded= 11 if HH_type==1
drop HHT_recoded
gen HHT_recoded= 11 if HH_type==1 & Sector==1
replace HHT_recoded=12 if HH_type==2 & Sector==1
replace HHT_recoded=21 if HH_type==1 & Sector==2

**Creating HH_ID
  egen HH_ID=concat( Qtr Visit FSU Smple_sg_sb_no Scnd_stg_strtm smpl_HH_no )
